...
> 一支准星，四种霓虹色——点对了消除，点错了分裂。
> Four neon colors, one crosshair — match to erase, miss to multiply.
...
- 底部中央技能条显示剩余次数，消耗时有一格 0.2 秒的熄灭过渡。
...
> ChromaSplit — cycle your crosshair through four neon colors to erase matching blocks, but every wrong click splits a block in two as they keep accelerating downward; survive as long as you can.
...
- Space triggers Reverse (twice per run): every block on screen instantly flips to upward motion at the same speed while gravity keeps pulling down, so they slow, hover, and fall back. Positions and horizontal velocities are left untouched.
...
| Space | Reverse (2 per run) |
| `?` button (top right) | Pause and show the rules |
